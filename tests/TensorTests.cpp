...
[[maybe_unused]] ut::suite tensor_constexpr_tests = [] {
  using namespace ut;
  {
  ...
  { // Tensor-matrix, tensor-vector multiplication tests
    ...
    "tensor_matrix_multiplication"_test = [data1] {
      ...
      static_assert(
        dllib::MatrixMultiplication(Tensor<2, 3, 2>(data1), Tensor<2, 3>(data2)) == Tensor<2, 3, 3>(expected));
    };
...
[[maybe_unused]] ut::suite tensor_runtime_tests = [] {
  using namespace ut;
  {
    int data1[3][4] = {
      {4, 7, 1, 3},
